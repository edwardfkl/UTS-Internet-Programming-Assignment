...
@section('content')
    ...
    <div class="mt-8 overflow-x-auto rounded-xl border border-zinc-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-zinc-200 text-sm">
            ...
            <tr>
                <x-admin.sort-th :label="__('admin.users.col_id')" column="id" :sort="$sort" :dir="$dir" route="admin.users.index"/>
                <x-admin.sort-th :label="__('admin.users.col_name')" column="name" :sort="$sort" :dir="$dir" route="admin.users.index"/>
                <x-admin.sort-th :label="__('admin.users.col_email')" column="email" :sort="$sort" :dir="$dir" route="admin.users.index"/>
                <x-admin.sort-th :label="__('admin.users.col_admin')" column="is_admin" :sort="$sort" :dir="$dir" route="admin.users.index"/>
                <x-admin.sort-th :label="__('admin.users.col_created')" column="created_at" :sort="$sort" :dir="$dir" route="admin.users.index"/>
                <x-admin.sort-th :label="__('admin.users.col_updated')" column="updated_at" :sort="$sort" :dir="$dir" route="admin.users.index"/>
            ...
            @empty
                <tr>
                    <td colspan="7" class="px-4 py-8 text-center text-zinc-500">{{ __('admin.users.empty') }}</td>
                </tr>
            @endforelse
            </tbody>
        </table>
